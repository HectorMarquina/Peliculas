...
<body>
    ...
    <table class="table">
        ...
        <tbody>
        <c:forEach items="${ listPelicula }" var="nombre" varStatus="estado">
            <tr>
                <td>${ estado.count }</td>
                <td>${ pelicula.id.nombre } ${ pelicula.id.descripcion }</td>
                ...
                <td>
                    <c:if test="${ pelicula.estado == 1 }">
                        ...
                        <button id="btn-delete-${ estado.count }" data-code="${ pelicula.id }" data-text="${ pelicula.id.nombre } ${ pelicula.id.descripcion }" type="button" class="btn btn-outline-danger" data-bs-toggle="modal" data-bs-target="#delete"><i class="fas fa-trash"></i> Eliminar</button>
                    </c:if>
                    <c:if test="${ pelicula.estado == 0 }">
    ...
    <div class="modal fade" id="details" tabindex="-1" aria-labelledby="exampleModalLabel2" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel2">Detalles del Peliculas</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
